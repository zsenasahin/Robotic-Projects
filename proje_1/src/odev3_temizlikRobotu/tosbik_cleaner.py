...
def move(velocity_publisher, speed, distance, is_forward):
        ...
        while True :
                ...
                if distance_moved >= distance - 0.05:
                    break
...
def rotate (velocity_publisher, angular_speed_degree, relative_angle_degree, clockwise):
    ...
    while True :
        ...
        if current_angle_degree >= relative_angle_degree - 1.0:  # 1 derece tolerans
            break
        loop_rate.sleep()
...
def gridClean(publisher):
    ...
    # 1, 1 koordinatına git
    go_to_goal(publisher, 1.0, 1.0)

    # İstenilen yönlendirmeyi ayarla
    setDesiredOrientation(publisher, 10, 90)
    move(publisher, 1.0, 9.0, True)

    for i in range(5):  # 1'den 8'e kadar döngü
       rotate(publisher, 10, 90, True)      
       move(publisher, 2.0, 1.0, True)
       rospy.sleep(1.0)
       rotate(publisher, 10, 90, True)
       move(publisher, 2.0, 9.0, True)
       rospy.sleep(1.0)
       rotate(publisher, 10, 90, False)
       move(publisher, 2.0, 1.0, True)
       rospy.sleep(1.0)
       rotate(publisher, 10.0, 90, False)
       move(publisher, 2.0, 9.0, True)
    pass
